...
module.exports = {
  meta: {
    docs: {
      ...
      recommended: true,
      url:
        "https://github.com/SpiffInc/eslint-plugin-spiff/blob/master/docs/rules/jsx-align.md",
    },
    type: "suggestion",
    fixable: "whitespace",
  },
  ...
  create(context) {
    ...
    function isMultiLine(jsxElement) {
      ...
      const maxLine = jsxElement.attributes.reduce(
        (prev, a) => Math.max(prev, sourceCode.getFirstToken(a).loc.start.line),
        -1 // no line
      )
    ...
    function getAlignmentColumn(jsxElement) {
      ...
      alignmentColumn = attributes
        .filter(
          (attribute) =>
            !isOnSameLine(attribute, attributes, jsxElement) &&
            !isBoolean(attribute) &&
            !isSpread(attribute)
        )
        .reduce(
